...
def create_feasibility_pdf(report_data: dict[str, Any], output_path: str | None = None) -> bytes:
    ...
    story.append(Paragraph(f"<b>Category:</b> {cat_name} | <b>Location:</b> {loc_name} | <b>Date:</b> {date_str}", subtitle_style))
    ...
    score_color = "#16A34A" if overall_score >= 70 else "#D97706" if overall_score >= 50 else "#DC2626"

    score_box_data = [
        [
            Paragraph("<b>Overall Feasibility Index</b>", ParagraphStyle("WhiteText", parent=bold_body_style, textColor=colors.white)),
            Paragraph(f"<b>{overall_score:.1f}%</b>", ParagraphStyle("ScoreText", parent=bold_body_style, fontSize=24, leading=28, textColor=colors.white, alignment=1)),
        ],
        [
            Paragraph(f"<b>Recommendation:</b> {recommendation}", ParagraphStyle("RecText", parent=body_style, textColor=colors.white)),
            Paragraph("High Potential" if overall_score >= 70 else "Moderate Viability", ParagraphStyle("SubText", parent=body_style, textColor=colors.white, alignment=1)),
        ]
    ]
    ...
    breakdown_data = [
        [Paragraph("<b>Domain Dimension</b>", bold_body_style), Paragraph("<b>Score</b>", bold_body_style), Paragraph("<b>Assessment</b>", bold_body_style)],
        [Paragraph("Market Demand & Reach", body_style), Paragraph(f"{m_score:.1f}%", body_style), Paragraph("Strong" if m_score >= 70 else "Moderate", body_style)],
        [Paragraph("Financial Viability & ROI", body_style), Paragraph(f"{f_score:.1f}%", body_style), Paragraph("Strong" if f_score >= 70 else "Moderate", body_style)],
        [Paragraph("Competition Threat Level", body_style), Paragraph(f"{c_score:.1f}%", body_style), Paragraph("Low Risk" if c_score >= 70 else "Moderate Risk", body_style)],
        [Paragraph("Infrastructure & Logistics", body_style), Paragraph(f"{i_score:.1f}%", body_style), Paragraph("Adequate" if i_score >= 60 else "Constrained", body_style)],
        [Paragraph("Risk Resilience", body_style), Paragraph(f"{r_score:.1f}%", body_style), Paragraph("High Resilience" if r_score >= 70 else "Needs Mitigation", body_style)],
    ]
    ...
    if fin_summary and isinstance(fin_summary, dict):
        ...
        fin_data = [
            [Paragraph("<b>Metric</b>", bold_body_style), Paragraph("<b>Amount (Rs.) / Value</b>", bold_body_style)],
            [Paragraph("Total Estimated Project Cost", body_style), Paragraph(f"Rs. {p_cost:,.2f}", body_style)],
            [Paragraph("Available Capital (Equity)", body_style), Paragraph(f"Rs. {a_cap:,.2f}", body_style)],
            [Paragraph("Recommended Bank Loan", body_style), Paragraph(f"Rs. {l_amt:,.2f}", body_style)],
            [Paragraph("Estimated Monthly Surplus", body_style), Paragraph(f"Rs. {m_sur:,.2f}", body_style)],
            [Paragraph("Estimated Monthly EMI", body_style), Paragraph(f"Rs. {m_emi:,.2f}", body_style)],
            [Paragraph("Net Monthly Surplus", body_style), Paragraph(f"Rs. {n_sur:,.2f}", body_style)],
        ]
    ...
    if schemes:
        ...
        sch_rows = [[Paragraph("<b>Scheme Name</b>", bold_body_style), Paragraph("<b>Subsidy %</b>", bold_body_style), Paragraph("<b>Status</b>", bold_body_style)]]
        for sch in schemes:
            ...
            sch_rows.append([Paragraph(name, body_style), Paragraph(sub_str, body_style), Paragraph(status, body_style)])
    ...
    # Footer note
    story.append(HRFlowable(width="100%", thickness=0.5, color=colors.HexColor("#94A3B8"), spaceBefore=15, spaceAfter=10))
    story.append(Paragraph(
        ...
        ParagraphStyle("FooterNote", parent=body_style, fontSize=8, leading=10, textColor=colors.HexColor("#64748B"), alignment=1),
    ))
