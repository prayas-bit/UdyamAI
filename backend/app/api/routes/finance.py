...
def create_expense(data: ExpenseCreate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
...
def get_expense_summary(
    ...
    for e in expenses:
        ...
        if e.is_recurring:
            recurring += e.amount
    return ExpenseSummary(total_expenses=total, by_category=by_cat, recurring_total=recurring, count=len(expenses))


@router.put("/expenses/{expense_id}", response_model=ExpenseResponse)
def update_expense(expense_id: UUID, data: ExpenseUpdate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
...
def delete_expense(expense_id: UUID, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    expense = _owned_or_404(session, Expense, expense_id, profile, "Expense")
    # Move to recycle bin
    item = RecycleBinItem(
        ...
        item_data=json.dumps({
            ...
            "amount": expense.amount, "date": str(expense.date),
...
def create_cashflow_entry(data: CashFlowEntryCreate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
...
def get_cashflow_summaries(profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    q = select(CashFlowSummary).where(CashFlowSummary.profile_id == profile.id).order_by(CashFlowSummary.period_start.desc())
    return session.exec(q).all()


@router.delete("/cashflow/{entry_id}")
def delete_cashflow_entry(entry_id: UUID, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
...
def get_savings_overview(profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    ...
    return SavingsOverview(goals=goals_resp, total_saved=total_saved, total_target=total_target, overall_progress=round(overall, 1))


@router.post("/savings/goals", response_model=SavingsGoalResponse)
def create_savings_goal(data: SavingsGoalCreate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
...
def update_savings_goal(goal_id: UUID, data: SavingsGoalUpdate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
...
def create_savings_transaction(goal_id: UUID, data: SavingsTransactionCreate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    goal = _owned_or_404(session, SavingsGoal, goal_id, profile, "Savings goal")
    ...
    goal.updated_at = datetime.utcnow()
    session.add(goal)
    txn = SavingsTransaction(goal_id=goal_id, amount=data.amount, transaction_type=data.transaction_type, notes=data.notes)
    session.add(txn)
    session.commit()
    session.refresh(txn)
    return txn
...
@router.get("/savings/goals/{goal_id}/transactions", response_model=list[SavingsTransactionResponse])
def list_savings_transactions(goal_id: UUID, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    _owned_or_404(session, SavingsGoal, goal_id, profile, "Savings goal")
    return session.exec(select(SavingsTransaction).where(SavingsTransaction.goal_id == goal_id).order_by(SavingsTransaction.date.desc())).all()
...
def list_budgets(profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    q = select(Budget).where(Budget.profile_id == profile.id, ~Budget.deleted).order_by(Budget.created_at.desc())
    ...
    for b in budgets:
        ...
        br.total_actual_expenses = sum(it.actual_amount for it in items if it.item_type == "expense")
        budgets_resp.append(br)
        total_inc += b.total_income_target
        total_exp += b.total_expense_target
        if b.status == "active":
            active += 1
    return BudgetOverview(budgets=budgets_resp, active_count=active, total_budgeted_income=total_inc, total_budgeted_expenses=total_exp)


@router.post("/budgets", response_model=BudgetResponse)
def create_budget(data: BudgetCreate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    budget = Budget(
        profile_id=profile.id, name=data.name, period_type=data.period_type,
        start_date=data.start_date, end_date=data.end_date,
        total_income_target=data.total_income_target, total_expense_target=data.total_expense_target,
        notes=data.notes,
    )
    session.add(budget)
    session.flush()
...
def update_budget(budget_id: UUID, data: BudgetUpdate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
...
def delete_budget(budget_id: UUID, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    budget = _owned_or_404(session, Budget, budget_id, profile, "Budget")
    budget.deleted = True
    session.add(budget)
    session.commit()
...
def list_debts(profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    ...
    total_emi = 0.0
    for d in debts:
        paid_pct = ((d.principal_amount - d.outstanding_amount) / d.principal_amount * 100) if d.principal_amount > 0 else 0
    ...
    return DebtOverview(debts=debts_resp, total_outstanding=total_outstanding, total_principal=total_principal, total_monthly_emi=total_emi)


@router.post("/debts", response_model=DebtResponse)
def create_debt(data: DebtCreate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
...
def update_debt(debt_id: UUID, data: DebtUpdate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
...
def create_debt_payment(debt_id: UUID, data: DebtPaymentCreate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    debt = _owned_or_404(session, Debt, debt_id, profile, "Debt")
...
def list_debt_payments(debt_id: UUID, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    _owned_or_404(session, Debt, debt_id, profile, "Debt")
    return session.exec(select(DebtPayment).where(DebtPayment.debt_id == debt_id).order_by(DebtPayment.payment_date.desc())).all()
...
def list_borrowings(profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    ...
    return BorrowingOverview(
        borrowings=items_resp, exploring_count=exploring, applied_count=applied,
        approved_count=approved, total_requested=total_req, total_approved=total_appr,
    )


@router.post("/borrowings", response_model=BorrowingResponse)
def create_borrowing(data: BorrowingCreate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
...
def update_borrowing(borrowing_id: UUID, data: BorrowingUpdate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
...
def delete_borrowing(borrowing_id: UUID, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    borrowing = _owned_or_404(session, Borrowing, borrowing_id, profile, "Borrowing")
...
def get_credit_overview(profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    q = select(CreditScore).where(CreditScore.profile_id == profile.id).order_by(CreditScore.recorded_date.desc())
    ...
    if latest:
        ...
        if latest.suggestions:
            try:
                suggestions_list = json.loads(latest.suggestions) if isinstance(latest.suggestions, str) else latest.suggestions
            except Exception:
                suggestions_list = [latest.suggestions]
    return CreditOverview(latest_score=latest, history=history, trend=trend, rating=rating, suggestions=suggestions_list)


@router.post("/credit", response_model=CreditScoreResponse)
def create_credit_score(data: CreditScoreCreate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    ...
    if score >= 800:
        ...
        rating = "poor"
    credit = CreditScore(profile_id=profile.id, rating=rating, **data.model_dump(exclude_unset=True))
    session.add(credit)
    session.commit()
...
def list_recycle_bin(profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    q = select(RecycleBinItem).where(RecycleBinItem.profile_id == profile.id, ~RecycleBinItem.restored).order_by(RecycleBinItem.deleted_at.desc())
    return session.exec(q).all()


@router.post("/recycle-bin/restore")
def restore_from_recycle_bin(data: RecycleBinRestoreRequest, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    restored_count = 0
...
def permanent_delete_recycle_bin(item_id: UUID, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
...
def get_privacy_overview(profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    ...
    return PrivacyOverview(consents=consents_resp, all_data_shared=data_shared, analytics_enabled=analytics, ai_processing_enabled=ai_proc)


@router.post("/privacy", response_model=PrivacyConsentResponse)
def update_privacy_consent(data: PrivacyConsentCreate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    # Find existing consent or create new
    existing = session.exec(
        select(PrivacyConsent).where(PrivacyConsent.profile_id == profile.id, PrivacyConsent.consent_type == data.consent_type)
    ).first()
    if existing:
    ...
    consent = PrivacyConsent(
        profile_id=profile.id, consent_type=data.consent_type, granted=data.granted,
...
def get_settings(profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    settings = session.exec(select(UserSettings).where(UserSettings.profile_id == profile.id)).first()
    if not settings:
...
def update_settings(data: SettingsUpdate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    settings = session.exec(select(UserSettings).where(UserSettings.profile_id == profile.id)).first()
    if not settings:
        settings = UserSettings(profile_id=profile.id)
    for k, v in data.model_dump(exclude_unset=True).items():
...
def get_profile(profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    return profile


@router.put("/profile", response_model=ProfileResponse)
def update_profile(data: ProfileUpdate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
    for k, v in data.model_dump(exclude_unset=True).items():
        setattr(profile, k, v)
...
def create_profile(data: ProfileUpdate, profile: Profile = Depends(get_current_profile), session: Session = Depends(get_session)):
