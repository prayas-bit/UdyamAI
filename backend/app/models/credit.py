...
class Borrowing(SQLModel, table=True):
    ...
    loan_type: str = Field(nullable=False, description="term_loan, working_capital, mudra, pmegp, gold_loan")
    ...
    status: str = Field(default="exploring", description="exploring, applied, under_review, approved, disbursed, rejected")
    ...
    documents_required: str | None = Field(default=None, description="Comma-separated list of required documents")
...
class CreditScore(SQLModel, table=True):
    ...
    provider: str = Field(default="estimated", description="estimated, cibil, equifax, crif, experian")
    ...
    suggestions: str | None = Field(default=None, description="JSON string of improvement suggestions")
